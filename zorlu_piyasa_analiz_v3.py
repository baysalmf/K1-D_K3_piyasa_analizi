...
def _debug(label: str, data):
    """Sadece debug_mode aktifken Streamlit ekranına veri basar."""
...
def _post_json(url, tgt_key, body, max_retries=5):
    ...
    if st.session_state.get("debug_mode"):
        st.sidebar.caption(f"🐛 POST → `{url.split('/')[-1]}`")
    for attempt in range(max_retries):
        ...
        if r.status_code == 429:
            wait = 15 * (attempt + 1)
            if st.session_state.get("debug_mode"):
                st.sidebar.warning(f"🐛 Rate limit (429) — {wait}s bekleniyor (deneme {attempt+1})")
            time.sleep(wait)
            continue
        if not r.ok:
        ...
        try:
            result = r.json()
            if st.session_state.get("debug_mode"):
                _debug(f"POST {url.split('/')[-1]} — yanıt (ilk 5 item)", result.get("items", result)[:5] if isinstance(result.get("items"), list) else result)
            return result
...
with st.sidebar:
    ...
    st.markdown("---")
    st.header("⚙️ Ayarlar")
    debug_mode = st.checkbox("🐛 Debug Mode", value=False, help="Etkinleştirildiğinde API istekleri ve ham veriler ekranda gösterilir.")
